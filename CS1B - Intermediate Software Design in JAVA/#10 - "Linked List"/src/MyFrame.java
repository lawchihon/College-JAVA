...
import javax.swing.*;
/**
 * One object of class MyFrame build a window to input the data of employee, add it into the list and print the list
 * @author johnlaw
 */
public class MyFrame extends JFrame implements ActionListener{
	...
	private void initialize(){
		...
		addPercentRaiseField = new JTextField("Type the percent raise here: ", 20);
	...
	public void actionPerformed(ActionEvent evt) {
		...
		try{
			if (evt.getSource() == addEmployee){
				...
				addEmployeeField();
				setEmployee = new JButton("Add Employee");
				add(setEmployee);
				...
				addAllMenuButton();
				screen = new JTextArea("Employee in the list was given a raise in salary!");
				this.add(screen);
	...
	/**
	 * Adds the text fields that created to raise employee salary
	 */
	public void addRaiseField(){
		this.add(addPercentRaiseField);
	}
	/**
	 * Gets the data from the text fields that created to raise employee salary
	 */
	public void getRaiseField(){
		percentRaise = Double.parseDouble(addPercentRaiseField.getText());
	}
	/**
	 * Removes all the text fields that created to raise employee salary
	 */
	public void removeRaiseField(){
		this.remove(addPercentRaiseField);
	}
	/**
	 * Initializes all the text fields that created to raise employee salary
	 */
